...
// reusable packages.
//
// The `.iOS`-only platform list is load-bearing, not stylistic (#123). The ogg
// and vorbis xcframeworks ship no watchOS slice, so adding `.watchOS` here — or
// letting the watch app link this product — breaks the watch build at link time
// with an error that names a missing architecture rather than the real cause.
// The watch supplies its own `AVPlayer`-backed `WatchRadioPlaybackEngine` and
// must keep doing so.
let package = Package(
