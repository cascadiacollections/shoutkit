...
private let audioIntentsLogger = Logger(subsystem: "Holmdel.App", category: "AudioIntents")
...
// Holmdel") that the schema exists to provide — it simply activates only where
...
/// opts into just the cases it actually plays. Holmdel only ever plays a
...
/// "Hey Siri, play KEXP on Holmdel" while driving. Deep links use
...
/// Holmdel" — because `AppShortcutPhrase` can only bind plain
...
struct PlayStationIntent: AudioPlaybackIntent {
    ...
    static let description = IntentDescription("Plays a radio station in Holmdel.")
...
/// `@AppIntent(schema: .audio.playAudio)` registers Holmdel as a system
...
/// without the app name being spoken and without Holmdel training a custom
...
/// play queue; Holmdel has none (station switches are immediate replacement,
...
/// looping). Holmdel's immediate single-station playback has none of these,
...
/// land. Holmdel has no queue — playing a station always replaces whatever
...
///
/// `@AppEntity(schema: .audio.liveRadioStation)` is what registers Holmdel as a
...
/// look a station up *once it already knows to ask Holmdel* (see DECISIONS.md,
/// 2026-07-15).
///
