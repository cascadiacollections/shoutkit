...
private let shortcutsLogger = Logger(subsystem: "Holmdel.App", category: "Shortcuts")
...
/// `HolmdelAudioIntents.swift` with the rest of the `AppSchema.audio` surface —
...
struct StationEntityQuery: EntityQuery, EntityStringQuery {
    private static let logger = Logger(subsystem: "Holmdel.App", category: "StationEntityQuery")
    ...
    func indexKnownStationsForSpotlight() async {
        ...
        if entities.isEmpty == false {
            ...
            // only — the explicit "on Holmdel" phrases still resolve from the
        ...
        HolmdelShortcuts.updateAppShortcutParameters()
    }
}
...
struct OpenHolmdelIntent: AppIntent {
    static let title: LocalizedStringResource = "Open Holmdel"
    static let description = IntentDescription("Opens Holmdel.")
...
struct GetCurrentPlaybackIntent: AppIntent {
    ...
    static let description = IntentDescription("Reports which station is currently playing in Holmdel.")
...
/// this to my favorites" is implicitly whatever Holmdel is playing, mirroring
...
struct HolmdelShortcuts: AppShortcutsProvider {
